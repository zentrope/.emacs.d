...
#+begin_src emacs-lisp
  (use-package emacs
    ...
    ;; (defun kfi/doom-light ()
    ;;   "Switch to the predefined light theme."
    ;;   (interactive)
    ;;   (kfi/unload-themes)
    ;;   (load-theme 'doom-one-light t))
...
#+begin_src emacs-lisp
  ...
  (use-package org
    ...
    (defun kfi/org-toggle-markers ()
      ...
      (interactive)
      (setq org-hide-emphasis-markers (not (bound-and-true-p org-hide-emphasis-markers)))
      (org-mode-restart))

    (defvar kfi/org-wide-fill-column 100
      "Fill column for org mode when used for working with tasks/tags.")
    (defvar kfi/org-wide-tags-column 85
      "Tags column for org mode when used for working with tasks/tags.")
    (defvar kfi/org-narrow-fill-column 80
      "Fill column for org mode when used for prose/documentation.")
    (defvar kfi/org-narrow-tags-column 65
      "Tags column for org mode when used for prose/documentation.")

    (setq org-src-window-setup 'split-window-below
          org-tags-column kfi/org-wide-tags-column
          org-support-shift-select t
          org-src-tab-actus-natively t
          org-src-window-setup 'current-window)

    (defun kfi/org-narrow-doc ()
      "Reset org fill params for a prose focussed mode."
      (interactive)
      (set-fill-column kfi/org-narrow-fill-column)
      (setq org-tags-column kfi/org-narrow-tags-column))

    (defun kfi/org-widen-doc ()
      "Reset org fill params for a task focussed mode."
      (interactive)
      (set-fill-column kfi/org-wide-fill-column)
      (setq org-tags-column kfi/org-wide-tags-column))

    (defun kfi/org-align-tags ()
      "Align tags if they've gotten messed up."
      (interactive)
      (org-align-tags t))

    (defun kfi/org-width ()
      "Set org-buffer frame to comfortable width."
      (interactive)
      (set-frame-width (selected-frame) (+ (or fill-column 100) 4)))

    (defun kfi/org-keys-setup ()
      (define-key org-mode-map (kbd "<f1>") 'org-tree-to-indirect-buffer)
      (define-key org-mode-map (kbd "<escape><escape>") 'org-tree-to-indirect-buffer)
      (define-key org-mode-map (kbd "C-c a") 'org-agenda)
      (define-key org-mode-map (kbd "C-c t") 'kfi/org-align-tags)
      (define-key org-mode-map (kbd "C-c w") 'kfi/org-width))
